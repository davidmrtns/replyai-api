...
from .reply_helpers import (
    _handle_evolutionapi_request,
    _handle_digisac_request,
    _handle_contact_can_receive_replies,
)
...
async def reply(
    ...
    try:
        ...
        if isinstance(request, EvolutionAPIRequest):
            if not await _handle_evolutionapi_request(
                request, company, message_client, is_audio, db
            ):
                return reply_result
        else:
        ...
        if pipeline:
            reply_result = await pipeline(
                response, is_audio, contact, company_data, assistant, db
            )
