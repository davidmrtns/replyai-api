...
def auth_callback(code: str, state: str, db: Session = Depends(get_db_session)):
    return generate_auth_callback(state, code, db)


@router.get("/{company_slug}/auth-link", include_in_schema=False)
def get_auth_link(company_slug: str, _: Company = Depends(check_company_access)):
    return generate_auth_link(company_slug)
...
def update_outlook_timezone(
    ...
    return update_outlook_timezone_service(outlook_client_id, request, company_id, db)
