...
async def get_digisac_client_from_db(
    ...
    digisac_client_db = db.query(DigisacClientDB).filter_by(company_id=company.id).first()
    if not digisac_client_db:
        raise HTTPException(status_code=404, detail="No Digisac client found for this company")
    else:
...
def get_department_from_db(
    ...
    department = db.query(Departamento).filter_by(id=department_id, digisac_client_id=digisac_client.id).first()
    if not department:
        raise HTTPException(status_code=404, detail="Department not found for this Digisac client")

    return department


def get_digisac_client(
        digisac_client_db: DigisacClientDB
) -> DigisacClient:
