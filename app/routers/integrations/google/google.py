...
async def auth_callback(
    ...
    if not code:
        raise HTTPException(status_code=400, detail='Code not available in the request.')

    company = db.query(Company).filter_by(slug=company_slug).first()
    if company:
        access_token, refresh_token, expires_in, user_email = generate_google_auth_credentials(code, company_slug)
