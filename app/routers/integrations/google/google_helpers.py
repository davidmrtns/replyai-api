...
USER_FRIENDLY_ERROR_DETAIL = 'Failed to authenticate with Google. Please try again later.'
...
async def get_google_calendar_client_from_db(
    ...
    google_calendar_client_db = db.query(GoogleCalendarClientDB).filter_by(company_id=company.id).first()
    if not google_calendar_client_db and raise_error_if_not_found:
        raise HTTPException(status_code=404, detail='No Google Calendar client found for this company')
    else:
        return google_calendar_client_db


def generate_google_auth_credentials(
        code: str,
        company_slug: str
):
    payload = {
    ...
    tokens_response = requests.post('https://oauth2.googleapis.com/token', data=payload, headers=headers)
