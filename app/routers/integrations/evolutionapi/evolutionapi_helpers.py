...
async def get_evolutionapi_client(
    ...
    evolutionapi_db = db.query(EvolutionAPIClientDB).filter_by(api_key=api_key, company_id=company.id).first()
    if evolutionapi_db:
        return EvolutionAPIClient(
            evolutionapi_db.api_key,
            evolutionapi_db.instance_name,
            80000
        )
    else:
        raise HTTPException(status_code=404, detail='No EvolutionAPI client found for this company')
    

async def add_evolutionapi_client_to_db(response: Response, company_id: int, db: Session) -> EvolutionAPIClientDB | None:
