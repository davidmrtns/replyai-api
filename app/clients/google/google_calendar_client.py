...
class GoogleCalendar(AgendaClient):
    def __init__(
        ...
        if credentials and credentials.expired and credentials.refresh_token:
            ...
            if credentials.token != access_token:
                ...
                refresh_token = credentials.refresh_token if credentials.refresh_token else refresh_token
    ...
    async def add_event(
        ...
        initial_datetime = datetime.strptime(date, '%Y-%m-%dT%H:%M:%S').astimezone(self.timezone)
        ...
        response = self.service.events().insert(calendarId=agenda_address, body=event).execute()
    ...
    async def confirm_event(
        ...
        try:
            ...
            if event:
                event['summary'] = f"CONFIRMADO - {event['summary']}" # TODO: localize string
                self.service.events().update(
                    calendarId=agenda_address,
                    eventId=event['id'],
                    body=event
                ).execute()
                return True
    ...
    async def reschedule_event(
        ...
        try:
            ...
            if event:
                intial_datetime = datetime.strptime(new_datetime, '%Y-%m-%dT%H:%M:%S').astimezone(self.timezone)
                end_datetime = intial_datetime + timedelta(minutes=self.event_duration)

                event['summary'] = f"REAGENDADO - {event['summary']}" # TODO: localize string
                event['start'] = {
        ...
        try:
            ...
            if event:
                if event_cancellation_type == 'delete':
                    ...
                    event['summary'] = f"CANCELADO - {event['summary']}" # TODO: localize string
                    self.service.events().update(
                        calendarId=agenda_address,
                        eventId=event['id'],
                        body=event
    ...
    async def _select_specific_event(
        ...
        events = self.service.events().list(
            calendarId=agenda_address,
            q=event_subject,
            timeMin=event_start_datetime
        ).execute()

        return events.get('items')[0] if events.get('items') and len(events.get('items')) > 0 else None
