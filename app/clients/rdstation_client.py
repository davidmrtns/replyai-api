...
class RDStationClient(CRMClient):
    ...
    def create_lead(
        ...
        request = {
            ...
            "deal_source": {"_id": self.default_deal_source_id},
            "contacts": [
                {
                    "name": contact_name,
                    "phones": [{"phone": contact_phone_number, "type": "cellphone"}],
                }
            ],
        }
    ...
    def change_stage(
        ...
        request = {"deal_stage_id": deal_stage_id}

        if user_id:
            request["deal"] = {"user_id": user_id}
