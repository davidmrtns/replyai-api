...
async def transfer_contact_to_department(
    ...
    with retornar_sessao() as db:
        ...
        if not assistant:
            raise FailedFunctionRunException(detail='Assistant not found in the database', function_name=transfer_contact_to_department.__name__)
        
        company: Company = assistant.company
        if not company:
            raise FailedFunctionRunException(detail='Company not found in the database', function_name=transfer_contact_to_department.__name__)

        message_client = create_message_client(company, db)
        if not message_client or not isinstance(message_client, DigisacClient):
            raise FailedFunctionRunException(detail='Message client not found or not supported for transfer', function_name=transfer_contact_to_department.__name__)

        current_thread = db.query(Thread).filter_by(thread_id=thread_id).first()
        if not current_thread:
            raise FailedFunctionRunException(detail='Thread not found in the database', function_name=transfer_contact_to_department.__name__)

        contact = current_thread.contact
        if not contact:
            raise FailedFunctionRunException(detail='Contact not found in the database', function_name=transfer_contact_to_department.__name__)

        department = db.query(Departamento).filter_by(codigo=department_code, id_empresa=company.id).first()
        if not department:
            raise FailedFunctionRunException(detail='Department not found in the database', function_name=transfer_contact_to_department.__name__)
